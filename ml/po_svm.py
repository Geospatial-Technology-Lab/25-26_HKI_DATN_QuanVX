...
class SVMPUMAOptimizer:
    """Bộ tối ưu hóa PUMA cho SVM Regressor."""
    ...
    def __init__(self, X: Union[np.ndarray, list] = None, y: Union[np.ndarray, list] = None, 
        ...
        """Khởi tạo bộ tối ưu hóa PUMA cho SVM Regressor."""
        ...
        # Lấy phạm vi tham số SVM từ file cấu hình
        param_ranges = get_param_ranges('svm')
        
        self.optimizer.set_param_ranges(param_ranges)
        self.optimizer.set_evaluate_function(self.evaluate_svm)
    
    def _create_svm_model(self, params: Dict[str, Any]) -> SVR:
        """Tạo SVR từ tham số - TÁN DỤNG LOGIC CŨ."""
        # Các tham số cơ bản
        svm_params = {
            'C': params.get('C', 1.0),
    ...
    def evaluate_svm(self, individual: Dict[str, Any], X_train: np.ndarray, X_test: np.ndarray, 
                    y_train: np.ndarray, y_test: np.ndarray) -> float:
        """Đánh giá fitness của SVM với các tham số - TÁN DỤNG HÀM CŨ."""
        # Tạo mô hình SVM Regressor
        model = self._create_svm_model(individual)
    ...
    def get_best_model(self) -> SVR:
        ...
        return self._create_svm_model(self.optimizer.best_individual)

def main() -> None:
    """Hàm chính để chạy quá trình tối ưu hóa SVM."""
    try:
        ...
        # Khởi tạo và chạy bộ tối ưu PUMA cho SVM
        print("Bắt đầu tối ưu hóa PUMA cho SVM...")
        svm_optimizer = SVMPUMAOptimizer()
        result = svm_optimizer.optimize()
        ...
        final_model.fit(svm_optimizer.optimizer.X_train_scaled, svm_optimizer.optimizer.y_train)
        y_pred = final_model.predict(svm_optimizer.optimizer.X_test_scaled)
        y_pred = np.clip(y_pred, 0, 1)  # Giới hạn dự đoán từ 0 đến 1
        
        # Tính toán và lưu metrics cuối cùng
        y_test = np.array(svm_optimizer.optimizer.y_test)
        ...
        print(f"\n{'='*60}")
        print("HOÀN THÀNH TỐI ƯU HÓA SVM")
