"""Random Forest PSO Optimizer - Tối ưu hóa Random Forest bằng PSO (Phiên bản tối giản)."""

import time
from sklearn.model_selection import train_test_split

# Import tất cả từ các module có sẵn
from pso_optimizer import RandomForestPSOOptimizer, RANDOM_SEED
...
def main() -> None:
    ...
    try:
        ...
        # === CHẠY TỐI ƯU HÓA ===
        rf_optimizer = RandomForestPSOOptimizer(X, y, 'regression', n_particles, n_iterations)
        
        start_time = time.time()
        result = rf_optimizer.optimize(verbose=True, print_table=True, save_csv=True, save_model=True)
        end_time = time.time()
