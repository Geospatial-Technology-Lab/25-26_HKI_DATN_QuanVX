"""SVM PSO Optimizer - Tối ưu hóa SVM bằng PSO (Phiên bản tối giản)."""

import time
from sklearn.model_selection import train_test_split

# Import tất cả từ các module có sẵn
from pso_optimizer import SVMPSOOptimizer, RANDOM_SEED
...
def main() -> None:
    ...
    try:
        ...
        # === CHẠY TỐI ƯU HÓA ===
        svm_optimizer = SVMPSOOptimizer(X, y, 'regression', n_particles, n_iterations)
        
        start_time = time.time()
        result = svm_optimizer.optimize(verbose=True, print_table=True, save_csv=True, save_model=True)
        end_time = time.time()
