"""Clean data utilities for flood prediction - FINAL CLEAN VERSION."""
...
# predict_to_tiff(models, Path("data.gdb"), "layer", Path("output"))
def calculate_raster_params_from_config(pixel_size: float = 0.00009) -> dict:
    """Calculate raster parameters from config bounds."""
    x_min, y_min, x_max, y_max = STUDY_AREA_BOUNDS
...
def calculate_raster_params(coords: np.ndarray, pixel_size: float = 0.00009) -> dict:
    """Calculate raster parameters from coordinates."""
...
def coords_to_pixels(coords: np.ndarray, raster_params: dict) -> Tuple[np.ndarray, np.ndarray]:
    """Convert coordinates to pixel indices (vectorized)."""
    bounds = raster_params['bounds']
    pixel_size = raster_params['pixel_size']
    height = raster_params['height']
    
    # Vectorized coordinate to pixel conversion
    cols = ((coords[:, 0] - bounds[0]) / pixel_size).astype(int)
    rows = ((bounds[3] - coords[:, 1]) / pixel_size).astype(int)
    
    # Filter valid pixels
    valid_mask = (rows >= 0) & (rows < height) & (cols >= 0) & (cols < raster_params['width'])
    
    return rows[valid_mask], cols[valid_mask], valid_mask


def predict_to_tiff_optimized(models: dict, file_path: Path, layer_name: str, 
                             output_dir: Path, chunk_size: int = 50000, 
                             pixel_size: float = 0.00009) -> None:
    """Optimized workflow: predict and create TIFF using config bounds."""
    
    print(f"🚀 Optimized TIFF prediction workflow")
    print(f"📦 Chunk size: {chunk_size:,}")
    print(f"📐 Pixel size: {pixel_size}")
    print(f"🗺️  Study area bounds: {STUDY_AREA_BOUNDS}")
    
    output_dir.mkdir(exist_ok=True)
    
    # Step 1: Calculate raster parameters from config bounds
    print("📍 Calculating raster dimensions from config...")
    raster_params = calculate_raster_params_from_config(pixel_size)
    
    print(f"📊 Raster: {raster_params['width']}x{raster_params['height']} = {raster_params['width'] * raster_params['height']:,} pixels")
    print(f"💾 RAM needed: ~{raster_params['width'] * raster_params['height'] * 4 / 1024**3:.2f} GB per model")
    ...
    for model_name, model in models.items():
        ...
        raster_array = np.full((raster_params['height'], raster_params['width']), 
                              np.nan, dtype=np.float32)
        
        # Step 3: Process data in chunks and fill raster
        chunk_pbar = tqdm(range(estimated_chunks), desc=f"Processing {model_name}")
        ...
        for chunk_idx in chunk_pbar:
            ...
            try:
                ...
                tqdm.write(f"⚠️  {model_name} prediction error: {e}")
                continue
            
            # Convert coordinates to pixels (vectorized)
            rows, cols, valid_mask = coords_to_pixels(coords, raster_params)
            
            if len(rows) > 0:
                # Fill raster array
                valid_predictions = predictions[valid_mask]
                raster_array[rows, cols] = valid_predictions
            
            # Memory cleanup
            del coords, features, predictions
        ...
        # Step 4: Save TIFF
        output_path = output_dir / f"{model_name}_flood_probability.tif"
        print(f"💾 Saving TIFF: {output_path.name}")
        
        # Count valid pixels
        valid_pixels = np.sum(~np.isnan(raster_array))
        print(f"📊 Valid pixels: {valid_pixels:,} / {raster_array.size:,} ({valid_pixels/raster_array.size*100:.1f}%)")
        ...
        # Free raster memory before next model
        del raster_array
        gc.collect()
